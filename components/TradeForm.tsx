    ...
    <div className="flex flex-col h-full bg-white dark:bg-dark-card border-l border-gray-200 dark:border-dark-border p-4 relative">
      <div className="flex justify-between items-center mb-4">
        <div className="flex bg-gray-100 dark:bg-slate-800 rounded border border-gray-200 dark:border-slate-700 p-0.5">
          ...
          <Tooltip content={(t.explanations as any).btnIsolated} position="bottom">
      ...
      <div className="flex bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded p-0.5 mb-4">
        ...
        <Tooltip content={(t.explanations as any).btnLimitOrder} position="bottom" className="flex-1">
           <button onClick={() => setType(OrderType.LIMIT)} className={`w-full py-1.5 text-xs font-bold rounded transition-all ${type === OrderType.LIMIT ? 'bg-slate-500 text-white shadow' : 'text-gray-500'}`}>{t.limit}</button>
        </Tooltip>
      </div>
      ...
      <div className="mb-1">
        ...
        <div className="relative flex items-center">
          ...
          <button onClick={() => setUnit(u => u === 'ASSET' ? 'USDC' : 'ASSET')} className="absolute right-0 h-full flex items-center pr-2 group">
            <Tooltip content={(t.explanations as any).btnUnitSwitch} position="top">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="flex flex-col">
          <Tooltip content={(t.explanations as any).btnSellShort} position="top" className="w-full mb-3">
             <button onClick={() => handleOrderClick(OrderSide.SELL)} className="w-full py-4 bg-trade-down hover:bg-red-600 text-white font-bold rounded-lg shadow-lg shadow-red-900/20">{t.sellShort}</button>
          </Tooltip>
          <div className="px-1 space-y-1">
